...
int main(int argc, const char* argv[]) {
        ...
        // const float start_alpha = 0.5;
        const float end_alpha = 0.01;
        size_t total = 1000;
        unsigned seed;
        __asm__ __volatile__ ("rdtsc" : "=a" (seed));
        // fix your seed here (if you need it)
	// seed = 1351712636;
	info << "alpha = " << end_alpha << std::endl;
        ...
        for (size_t n = 1; n <= total; n++) {
                ...
                float alpha = end_alpha; // start_alpha + (n-1) * (end_alpha - start_alpha) / total;
