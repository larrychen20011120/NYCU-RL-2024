...
class DQNBaseAgent(ABC):
	def __init__(self, config):
		...
		# save limit
		self.saved_models = []
		self.max_n_models = config["max_n_models"]
	...
	def save(self, save_path):
		...
		if len(self.saved_models) > self.max_n_models:
			# Remove the oldest file
			oldest_model = self.saved_models.pop(0)
			if os.path.exists(oldest_model):
				os.remove(oldest_model)
				print(f"Deleted old model: {oldest_model}")
		...
		while self.total_time_step <= self.training_steps:
			...
			for i in range(self.num_envs):
				if terminates[i] or truncates[i]:
					...
					episode_idx += 1
					print(f"[{self.total_time_step}/{self.training_steps}]  episode: {episode_idx}  episode reward: {episode_rewards[i]}  episode len: {episode_lens[i]}  epsilon: {self.epsilon}")
					episode_rewards[i] = 0
					episode_lens[i] = 0
