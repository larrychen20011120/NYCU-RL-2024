	...
	def update(self):
		...
		for _ in range(self.update_count):
			for start in range(0, sample_count, self.batch_size):
				...
				_, logp, value, entropy = self.net(ob_train_batch, a=ac_train_batch.detach())

				# calculate policy loss
				logp = logp.reshape(-1)
				logp_pi_train_batch = logp_pi_train_batch.reshape(-1)
				...
				surrogate_loss = -torch.mean(torch.min(surrogate_loss1, surrogate_loss2))

				# calculate value loss
				value_criterion = nn.MSELoss()
				v_loss = torch.mean(value_criterion(value, return_train_batch.detach()))
