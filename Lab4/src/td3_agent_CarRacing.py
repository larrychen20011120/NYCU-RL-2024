...
class CarRacingTD3Agent(TD3BaseAgent):
	...
	def decide_agent_actions(self, state, sigma=0.0, brake_rate=0.015):
		...
		with torch.no_grad():
			...
			action = self.actor_net(state).cpu() + sigma * self.noise.generate()
			action = action.numpy()
	...
	def update_behavior_network(self):
		...
		with torch.no_grad():
			# select action a_next from target actor network and add noise for smoothing
			a_next = self.target_actor_net(next_state) + torch.tensor(self.noise.generate(), dtype=torch.float32, device=self.device)

			# no smooth regulatization
			...
			# select min q value from q_next1 and q_next2 (double Q learning)
			min_q = torch.min(q_next1, q_next2)
			q_target = reward + self.gamma * min_q * (1 - done)
